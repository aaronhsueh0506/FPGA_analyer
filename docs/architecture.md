...
### Phase 43 (v0.43) — 自適應熱力圖 + 散佈圖資源預算分區

重做「資料散佈分析」的兩張圖，解決原本 `QUANT_STEP=5` 寫死量化造成的問題：mode 類小位寬欄位（如 3-bit 值 0..7）被折成只剩 {0,5}；blob 半徑難調，密集離散資料不是糊成一團紅就是縮成孤立點。

- **新增純資料模組 `heatmapData.ts`**：把分箱與自適應邏輯抽成無 React 的純函式。`computeAxisStats`（相異值/min/max/bitMax）、`buildBins`（低基數逐值成格、高基數用 `{1,2,5}×10^k` nice step 量化）、`percentileCap`（最近排名百分位）、`limitCurve`（資源上限曲線，可 clamp 到框內供區域填色）、`binRange`（tooltip 區間字串）。
- **`Heatmap2D` 改用 ECharts 原生 `type:'heatmap'`**：兩種模式——「精準格狀」（每相異值一格、格數少時格內顯示數字）與「平滑模糊」（保留 heatmap.js，半徑用重疊公式 `min(maxSpacing×1.2, min(W,H)×8%)` 取回 KDE 連續感、`minOpacity` 0.08 讓稀疏密度可見）。「自動」即精準格狀（已內建自適應，故移除原先重複的「平滑細格」）。
- **對數著色**：cell 顏色用 `log1p(count)`、visualMap 上限 `log1p(cappedMax)`，低 count 也清楚可見並與空白(0)區分；移除 cell 邊框消除格線間隙。
- **控制項**：顯示模式（自動/精準格狀/平滑模糊）、密度（自動/較細/較粗/更粗，調 nice-step bin 數）、色階上限（100/95/90/80% 百分位 cap，防極端值洗版）、資源上限線開關。
- **0.5MB 資源上限**：依 `W × H × fmt < 500000`（fmt = 1/2/4 bytes 對應 8/16/32-bit）。散佈圖以巢狀底色分區呈現（綠/黃/紅，靠原點 = 連 32-bit 都放得下、外圈 = 僅 8-bit、再外 = 超預算）；熱力圖以對齊格線的隱藏 value 副軸疊加同樣三條虛線。
- **X/Y 對調按鈕**：`DualRegisterChart` 新增按鈕交換 X/Y 欄位，讓大範圍欄位可放到較寬的橫軸，配合圖的寬扁長寬比。
- **曾嘗試後移除**：3D 長條圖（echarts-gl）與 Log 軸刻度切換，實測對此資料觀察無助益，皆已移除。

**主要改動檔案**：`heatmapData.ts`（新建）、`Heatmap2D.tsx`（重寫）、`Scatter.tsx`（資源分區 + 加高 600px + 細格線）、`DualRegisterChart.tsx`（傳入 bit 寬度 + X/Y 對調）、`en.json` / `zh-TW.json`（新增 i18n key）。

**v0.43.1 補充**：熱力圖工具列新增「字體大小」控制（小/中/大/特大，預設「中」≈ 原本外觀）。單一控制同時縮放「格內 count 數字」與「X/Y 軸刻度標籤」字體（軸標題不變），用 `FONT_PX` 對照表映射到 `count` / `axis` 兩個 px 值；設定屬顯示偏好，切換 X/Y 軸或 case 範圍時不重置。

**v0.43.2 補充**：將「字體大小」改為數字 +/− 微調框（可直接輸入，夾在 6–28px，預設 10），且**只作用於格內 count 數字**；X/Y 軸刻度標籤改回固定 10px。另新增「顯示數字」勾選框（預設開）可整個關閉格內數字，關閉時字體微調框 disabled 並變灰。實作上移除 `FONT_PX` 對照表，改用 `COUNT_FONT_MIN/MAX/DEFAULT`、`AXIS_FONT_PX` 常數與 `clampFont`；新增 `showNumbers` state 併入 `showLabels` 判斷與 grid `key`。

**v0.44 補充**：為「數值 (magnitude)」bit field 新增每欄位的**解讀格式** `ValueFormat = 'uint' | 'sint' | 'fp32'`，存在 `FieldRange.format`（沿用 rangeMap → localStorage，每個 register 一份）。後端永遠存 unsigned 原始值，格式只是前端「怎麼解讀」。共用 helper 放在 `useBitFieldTypes.ts`：`interpretValue(raw, width, format)`（sint 用二補數、fp32 用 `Uint32Array`/`Float32Array` 重解讀，皆以 `2 ** w` 避免 `1<<32` 溢位）與 `formatBounds(width, format)`（uint `0~2^w-1`、sint `-2^(w-1)~2^(w-1)-1`）。控制 UI：在 magnitude 的有效範圍彈窗（`RangePopup`）最上方加一個 `<select>` 單選（unsigned/signed，`width===32` 才有 fp32）；選 fp32 時 min/max 輸入 disabled 並清空；signed 時驗證與 `<input min/max>` 改用 `formatBounds`，允許負的下限。**套用點**：統計（`StatsPanel` 先 `interpretValue` 再算 `computeStats`、範圍過濾與直方圖；自訂 min/max 或 `formatBounds` 作為直方圖上下限）、總覽涵蓋率（`OverallPanel` 解讀後算 min/max/不同值/涵蓋率，理論範圍用 `formatBounds`，fp32 該欄顯示「FP32」且涵蓋率顯示「—」）、超範圍警告（`Results.tsx` 與 `ResultsTable` 高亮判斷比較前先 `interpretValue`）。**不套用**：主資料表的儲存格數值與 2D 熱力圖/散佈圖維持顯示原始 unsigned 值。圖表元件 `Histogram` / `ValueCurve` 介面由 `interpretAs` 改為 `isFloat`（值已在上游解讀；整數小值分支改用 `minValue` 偏移以支援負值），並移除統計分頁內舊的 Int/FP32 切換（連同 i18n `interpretInt`/`interpretFP32`/`fp32OnlyFor32bit`）。另：2D 熱力圖格內字體上限 28 → 40px。

---

### Phase 42 (v0.42) — Mode Bit Field 片段有效範圍

- **`FieldRange` 型別擴充**：`useBitFieldTypes.ts` 的 `FieldRange` 新增 `segments?: string`（原始輸入字串）與 `parsedSegments?: [number, number][]`（解析後的排序非重疊區段陣列）；新增 `parseSegments()`、`isValueInRange()`、`validValueCount()` 三個匯出函式供多元件共用。
- **`ModeRangePopup`（替換 `SegmentPopup`）**：mode 類 bit field 的有效範圍 popup 改為統一介面——上方為 min/max 輸入框，下方「啟用片段範圍」勾選框展開 segment 文字輸入；兩者同時可填，segment 值須落在 min/max 區間內，不符合時即時顯示錯誤；儲存時同時寫入 `{ min, max, segments, parsedSegments }`。
- **驗證邏輯**：`parseSegments()` 驗證格式（`a-b` 或 `c` 以逗號分隔）、數值不超出 bit 寬度、區段 start ≤ end、無重疊；若有 min/max，另外驗證每個 segment 值都在 `[min, max]` 範圍內；min/max 改變時 segment 驗證即時重算。
- **Overall Panel 更新**：magnitude 涵蓋率計算改用 `validValueCount(range, width)` 作為分母（含 min/max 客製化）；mode 欄位若有 `parsedSegments`，新增「Mode Segment Coverage」表格（合法片段、有效空間、不同值個數、涵蓋率）；組合分析只計算所有選取欄位值均通過 `isValueInRange` 的 row，並顯示排除筆數。
- **Histogram 直方圖更新**：`Histogram.tsx` 新增 `allowedValues?: number[]` prop；mode 欄位有 `parsedSegments` 且 maxValue ≤ 20 時，只建立合法位置的 bar（如 `0-2, 5, 7-9` 只顯示 0/1/2/5/7/8/9，不出現空 bar 佔位）；所有直方圖改用 `isValueInRange` 過濾值，不再受限於 min/max 連續區間。

**主要改動檔案**：`useBitFieldTypes.ts`（資料模型 + 輔助函式）、`BitFieldTypeModal.tsx`（ModeRangePopup）、`OverallPanel.tsx`（覆蓋率 + Mode Segment Coverage + 組合過濾）、`StatsPanel.tsx`（histogram 過濾）、`Histogram.tsx`（allowedValues）、`en.json` / `zh-TW.json`（新增 segment 相關 i18n key）。

---

### Phase 41 (v0.41) — 值篩選支援多條件 AND

將值篩選從單一條件升級為多條件並行（AND 邏輯）：

- 工具列 checkbox 開啟後，工具列下方展開條件清單（欄位下拉 + `=` + 數值輸入 + `×` 移除）。
- `+ 新增條件` 按鈕可新增任意數量條件；條件數 > 1 時才顯示 `×` 移除按鈕。
- 篩選邏輯：所有「已填值且合法」的條件同時成立（`active.every(...)`），空值條件自動略過。
- checkbox 旁以 `(N)` 顯示目前有效條件數。

**改動檔案**：`frontend/src/components/results/ResultsTable.tsx`（`filterConditions` array state + `addCondition / removeCondition / updateCondition` helpers + 條件清單 JSX）；i18n 新增 `results.valueFilterAdd`。
