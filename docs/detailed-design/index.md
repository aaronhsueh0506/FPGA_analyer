...
本版（v0.44）的詳細設計已涵蓋 magnitude bit field 的**每欄位數值解讀格式**（unsigned / signed / fp32）：後端永遠儲存 unsigned 原始值，格式僅改變前端如何解讀（signed 用二補數、fp32 以 IEEE-754 重解讀），並套用於統計、總覽涵蓋率與超範圍警告。
