...
# Step 3: Check if port 8000 is already in use and release it
echo "[Step 3] Checking port 8000..."
PIDS=$(lsof -ti :8000 2>/dev/null || true)
if [ -n "${PIDS}" ]; then
    echo "         Port 8000 occupied (PID: ${PIDS}). Stopping old process..."
...
echo "[Step 4] Starting backend at http://localhost:8000"
